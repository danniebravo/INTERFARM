...
<div class="space-y-6">
    ...
    <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
            <h1 class="text-3xl font-extrabold tracking-tight text-gray-900">Crear lote</h1>
            <p class="text-sm text-gray-500 mt-1">
                Registra un lote y, si quieres, dibuja su croquis directamente sobre el mapa.
            </p>
        </div>
    ...
    <form method="POST" action="{{ route('lots.store') }}" class="space-y-6">
        ...
        <div class="glass rounded-[28px] p-6">
            <div class="mb-5">
                ...
                <p class="text-sm text-gray-500 mt-1">Datos básicos para identificar el lote.</p>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-5">
                ...
                <div>
                    ...
                    <p class="mt-1 text-xs font-semibold text-gray-500">Este código no podrá cambiarse después de crear el lote.</p>
                    @error('code')
                        <span class="mt-1 block text-sm text-red-600">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select name="type"
                        ...
                        <option value="pradera" {{ old('type') === 'pradera' ? 'selected' : '' }}>Pradera</option>
                        <option value="potrero" {{ old('type') === 'potrero' ? 'selected' : '' }}>Potrero</option>
                        <option value="corral" {{ old('type') === 'corral' ? 'selected' : '' }}>Corral</option>
                        <option value="descanso" {{ old('type') === 'descanso' ? 'selected' : '' }}>Descanso</option>
            ...
            <div class="mt-5">
                ...
                <textarea name="description"
                          ...
                          placeholder="Notas del lote, observaciones, uso o detalles...">{{ old('description') }}</textarea>
                @error('description')
                    <span class="mt-1 block text-sm text-red-600">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="mt-5">
                ...
                <textarea name="notes"
                          rows="4"
                          class="w-full rounded-xl border border-black/10 bg-white/70 px-4 py-3 outline-none focus:border-brand/40"
                          placeholder="Anota aquí cualquier detalle, recordatorio o pendiente del lote...">{{ old('notes') }}</textarea>
                @error('notes')
                    <span class="mt-1 block text-sm text-red-600">{{ $message }}</span>
                @enderror
            </div>
        ...
        <div class="lot-map-shell">
            <div class="mb-5">
                ...
                <p class="text-sm text-gray-500 mt-1">
                    Puedes dejarlo vacío o dibujar el polígono del lote con clics sobre el mapa.
                </p>
            </div>
            ...
            <div class="lot-map-help mb-4" id="mapHelpText">
                Puedes buscar una dirección o vereda para centrar el mapa sin usar GPS. Luego activa el dibujo y marca los puntos del lote.
            </div>
            ...
            <div class="lot-map-box">
                <div id="lotMapOverlay" class="lot-map-overlay">
                    <div class="max-w-lg text-center">
                        <div class="text-xl font-extrabold text-gray-900">Mapa opcional del lote</div>
                        <p class="text-sm text-gray-600 mt-3">
                            Puedes buscar una ubicación, usar tu GPS si estás en el sitio o mover el mapa manualmente para dibujar el lote.
                        </p>
                    </div>
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-5 mt-5">
                ...
                <div>
                    ...
                    <input type="number"
                           ...
                           value="{{ old('area_calculated') }}"
                           class="w-full rounded-xl border border-black/10 bg-white/70 px-4 py-2.5 outline-none focus:border-brand/40"
                           placeholder="Se calcula con el croquis">
                ...
                <div>
                    ...
                    <input type="number"
                           ...
                           id="centerLatInput"
                           name="center_lat"
                           value="{{ old('center_lat') }}"
                           class="w-full rounded-xl border border-black/10 bg-white/70 px-4 py-2.5 outline-none focus:border-brand/40"
                           placeholder="Se llena con el mapa">
                    @error('center_lat')
                        <span class="mt-1 block text-sm text-red-600">{{ $message }}</span>
                    @enderror
                </div>
            ...
            <input type="hidden" id="polygonJsonInput" name="polygon_json" value="{{ old('polygon_json') }}">
        </div>
...
<script>
    ...
    function initLotCreateMap() {
        ...
        const focusSearchedLocation = (location, viewport = null, title = 'Ubicación buscada') => {
            ...
            helpText.textContent = 'Ubicación encontrada. Ahora puedes acercar el mapa y empezar a dibujar el lote.';
        };
        ...
        const activateMap = () => {
            if (mapLoaded) {
                ...
                helpText.textContent = 'Mapa activo. Puedes buscar una ubicación, moverte manualmente o dibujar tu lote.';
                return;
            }
            ...
            mapLoaded = true;
            overlay.classList.add('hidden');
            helpText.textContent = 'Mapa activo. Busca una ubicación o pulsa "Empezar a dibujar" para marcar el lote.';
            updateButtons();
        };
            ...
            try {
                ...
                if (Array.isArray(parsed) && parsed.length >= 3) {
                    ...
                    lotMap.fitBounds(bounds);
                        ...
                        helpText.textContent = 'Se cargó el croquis anterior. Puedes arrastrar sus puntos para reubicarlos.';
                }
            } catch (error) {
...
<script
    src="https://maps.googleapis.com/maps/api/js?key={{ $googleMapsApiKey }}&libraries=geometry&callback=initLotCreateMap"
    async
    defer
></script>
